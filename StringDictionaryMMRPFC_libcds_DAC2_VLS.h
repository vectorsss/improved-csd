...
#ifndef _STRINGDICTIONARY_MMRPFC_LIBCDS_DAC2_VLS_H
#define _STRINGDICTIONARY_MMRPFC_LIBCDS_DAC2_VLS_H
...
class StringDictionaryMMRPFCDAC2VLS : public StringDictionary
{     
	public:
		/** Generic Constructor. */
		StringDictionaryMMRPFCDAC2VLS();

		/** Class Constructor.
		    @param it: iterator scanning the original set of strings.
		*/
		StringDictionaryMMRPFCDAC2VLS(IteratorDictString *it);
...
//		DAC_VLS *textStrings;	//! Front-Coding of all remaining strings
		
		DAC *llcp;	//! Positional index to the strings representation
		DAC *rlcp;	//! Positional index to the strings representation
...
//		BitSequence * selPointers;
		...
		*/
		 ...
		 * TODO
		*/
		int doSearchString(uchar * string, uint strLen, uint lpos, uint rpos, uint l, uint r);
		...
		*/
		...
		inline uint decodeCompareString(uint dictmPos, uchar *compared, uint *pos);
		inline uint decodeString(uchar *str, uint pos, uint limit);
