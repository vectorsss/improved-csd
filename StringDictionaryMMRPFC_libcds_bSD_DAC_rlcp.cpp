...
StringDictionaryMMRPFCbSDDACRlcp::StringDictionaryMMRPFCbSDDACRlcp()
{
	this->type = MMRPFCbSDDACRLCP;
	this->elements = 0;
	this->maxlength = 0;
}

StringDictionaryMMRPFCbSDDACRlcp::StringDictionaryMMRPFCbSDDACRlcp(IteratorDictString *it)
{
...
inline uint
StringDictionaryMMRPFCbSDDACRlcp::decodeString(uchar *str, uchar *ptr, uint limitstring, uint limitbytes)
{
...
uint 
StringDictionaryMMRPFCbSDDACRlcp::locate(uchar *str, uint strLen)
{
...
uchar *
StringDictionaryMMRPFCbSDDACRlcp::extract(size_t id, uint *strLen)
{
...
IteratorDictID*
StringDictionaryMMRPFCbSDDACRlcp::locatePrefix(uchar *str, uint strLen)
{
...
IteratorDictID*
StringDictionaryMMRPFCbSDDACRlcp::locateSubstr(uchar *str, uint strLen)
{
...
uint 
StringDictionaryMMRPFCbSDDACRlcp::locateRank(uint rank)
{
	return rank;
}

IteratorDictString*
StringDictionaryMMRPFCbSDDACRlcp::extractPrefix(uchar *str, uint strLen)
{
...
IteratorDictString*
StringDictionaryMMRPFCbSDDACRlcp::extractTable()
{
...
IteratorDictString*
StringDictionaryMMRPFCbSDDACRlcp::extractSubstr(uchar *str, uint strLen)
{
...
uchar *
StringDictionaryMMRPFCbSDDACRlcp::extractRank(uint rank, uint *strLen)
{
	return extract(rank, strLen);
}

size_t 
StringDictionaryMMRPFCbSDDACRlcp::getSize()
{
	return (bytesStrings*sizeof(uchar))+this->selPointers->getSize()+rlcp->getSize()+sizeof(StringDictionaryMMRPFCbSDDACRlcp)+rp->getSize();
}

void 
StringDictionaryMMRPFCbSDDACRlcp::save(ofstream &out)
{
...
StringDictionary*
StringDictionaryMMRPFCbSDDACRlcp::load(ifstream &in)
{
	size_t type = loadValue<uint32_t>(in);
	if(type != MMRPFCbSDDACRLCP) return NULL;

	cout << "Reading MMRPFCbSDDACRLCP" <<endl;

	StringDictionaryMMRPFCbSDDACRlcp *dict = new StringDictionaryMMRPFCbSDDACRlcp();

	dict->type = MMRPFCbSDDACRLCP;
...
//void
//StringDictionaryMMRPFCbSDDACRlcp::decodeNextString(uchar **ptr, uint lenPrefix, uchar *str, uint *strLen)
//{
//	uint lenSuffix;
//
...
//uint
//StringDictionaryMMRPFCbSDDACRlcp::searchPrefix(uchar **ptr, uint scanneable, uchar *decoded, uint *decLen, uchar *str, uint strLen)
//{
...
//
//uint
//StringDictionaryMMRPFCbSDDACRlcp::searchDistinctPrefix(uchar *ptr, uint scanneable, uchar *decoded, uint *decLen, uchar *str, uint strLen)
//{
//	uint id = 1;
//	uint lenPrefix;
//
...
void StringDictionaryMMRPFCbSDDACRlcp::printInfo() {
...
//    printf("XBWInt -> %lu\n", this->xbw->getSize());
//    printf("----------------------------\n");
    printf("MMPFC structure: %lu\n", sizeof(StringDictionaryMMRPFCbSDDACRlcp));
    printf("TOTAL  -> %lu\n", this->getSize());
//    printf("****************************\n");
//    printf("\n\n");
//
...
void StringDictionaryMMRPFCbSDDACRlcp::buildLcps(uchar *dict, uint *pointers, vector<size_t>*llcp, vector<size_t>*rlcp, uint l, uint r) {
    uint m = (l + r) / 2;
//    uint level = bits(r-l);
//    if (level > 10)
...
/*
inline uint
StringDictionaryMMRPFCbSDDACRlcp::decodeCompareString(uchar *ptr, uchar *pattern, uint *pos) 
{
...
inline uint
StringDictionaryMMRPFCbSDDACRlcp::decodeCompareString(uchar *ptr, uchar *pattern, uint *pos, int len) 
{
...
inline int 
StringDictionaryMMRPFCbSDDACRlcp::doSearchString(uchar * string, uint len, uint lpos, uint rpos, uint l, uint r) {
