...
//			printf("looking for %d\n", strings[j]);
			uint id = dict->locate(strings[j], lengths[j]);
	//		printf("locate (%s) = %d; extract = %s\n", strings[j], id, dict->extract(id, &foo));
		}
...
//		 	printf("extract (%d) = %s; locate = %d\n", ids[j], str, dict->locate(str, strLen));
			delete [] str;
		}
