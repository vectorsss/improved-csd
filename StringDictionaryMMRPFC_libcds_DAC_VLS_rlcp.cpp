...
StringDictionaryMMRPFCDACVLSRlcp::StringDictionaryMMRPFCDACVLSRlcp()
{
	this->type = MMRPFCDACVLSRLCP;
	this->elements = 0;
	this->maxlength = 0;
}

StringDictionaryMMRPFCDACVLSRlcp::StringDictionaryMMRPFCDACVLSRlcp(IteratorDictString *it)
{
...
inline uint
StringDictionaryMMRPFCDACVLSRlcp::decodeString(uchar *str, uint pos, uint limit)
{
...
uint 
StringDictionaryMMRPFCDACVLSRlcp::locate(uchar *str, uint strLen)
{
...
uchar *
StringDictionaryMMRPFCDACVLSRlcp::extract(size_t id, uint *strLen)
{
...
IteratorDictID*
StringDictionaryMMRPFCDACVLSRlcp::locatePrefix(uchar *str, uint strLen)
{
...
IteratorDictID*
StringDictionaryMMRPFCDACVLSRlcp::locateSubstr(uchar *str, uint strLen)
{
...
uint 
StringDictionaryMMRPFCDACVLSRlcp::locateRank(uint rank)
{
	return rank;
}

IteratorDictString*
StringDictionaryMMRPFCDACVLSRlcp::extractPrefix(uchar *str, uint strLen)
{
...
IteratorDictString*
StringDictionaryMMRPFCDACVLSRlcp::extractTable()
{
...
IteratorDictString*
StringDictionaryMMRPFCDACVLSRlcp::extractSubstr(uchar *str, uint strLen)
{
...
uchar *
StringDictionaryMMRPFCDACVLSRlcp::extractRank(uint rank, uint *strLen)
{
	return extract(rank, strLen);
}

size_t 
StringDictionaryMMRPFCDACVLSRlcp::getSize()
{
	return llcp->getSize()+sizeof(StringDictionaryMMRPFCDACVLSRlcp)+rp->getSize();
}

void 
StringDictionaryMMRPFCDACVLSRlcp::save(ofstream &out)
{
	...
	rp->save(out, MMRPFCDACVLSRLCP);
}



StringDictionary*
StringDictionaryMMRPFCDACVLSRlcp::load(ifstream &in)
{
	size_t type = loadValue<uint32_t>(in);
	if(type != MMRPFCDACVLSRLCP) return NULL;

	cout << "Reading MMRPFCDACVLSRLCP" <<endl;

	StringDictionaryMMRPFCDACVLSRlcp *dict = new StringDictionaryMMRPFCDACVLSRlcp();

	dict->type = MMRPFCDACVLSRLCP;
...
//void
//StringDictionaryMMRPFCDACVLSRlcp::decodeNextString(uchar **ptr, uint lenPrefix, uchar *str, uint *strLen)
//{
//	uint lenSuffix;
//
...
//uint
//StringDictionaryMMRPFCDACVLSRlcp::searchPrefix(uchar **ptr, uint scanneable, uchar *decoded, uint *decLen, uchar *str, uint strLen)
//{
...
//
//uint
//StringDictionaryMMRPFCDACVLSRlcp::searchDistinctPrefix(uchar *ptr, uint scanneable, uchar *decoded, uint *decLen, uchar *str, uint strLen)
//{
//	uint id = 1;
//	uint lenPrefix;
//
...
void StringDictionaryMMRPFCDACVLSRlcp::printInfo() {
...
//    printf("XBWInt -> %lu\n", this->xbw->getSize());
//    printf("----------------------------\n");
    printf("MMPFC structure: %lu\n", sizeof(StringDictionaryMMRPFCDACVLSRlcp));
    printf("TOTAL  -> %lu\n", this->getSize());
//    printf("****************************\n");
//    printf("\n\n");
//
...
void StringDictionaryMMRPFCDACVLSRlcp::buildLcps(uchar *dict, uint *pointers, vector<size_t>*llcp, vector<size_t>*rlcp, uint l, uint r) {
    uint m = (l + r) / 2;
//    uint level = bits(r-l);
//    if (level > 10)
...
/*
inline uint
StringDictionaryMMRPFCDACVLSRlcp::decodeCompareString(uchar *ptr, uchar *pattern, uint *pos)
{
...
inline uint
StringDictionaryMMRPFCDACVLSRlcp::decodeCompareString(uint dictmPos, uchar *pattern, uint *pos)
{
...
inline int 
StringDictionaryMMRPFCDACVLSRlcp::doSearchString(uchar * string, uint len, uint lpos, uint rpos, uint l, uint r) {
...
inline int StringDictionaryMMRPFCDACVLSRlcp::compareNewPattern(uchar *pattern, uchar *compared, uint *pos) {
	uint p=*pos;
