...
#include "StringDictionaryMMPFC.h"

StringDictionaryMMPFC::StringDictionaryMMPFC()
{
	this->type = MMPFC;
	this->elements = 0;
	this->maxlength = 0;
...
StringDictionaryMMPFC::StringDictionaryMMPFC(IteratorDictString *it)
{
...
//		printf("%s %d\n", strCurrent, lenCurrent);
	...
	printf("Original text length (with separators) : %d\n", processed);

	printf("Found %d elements in dict. Added 2 markers\n", elements);
...
uint 
StringDictionaryMMPFC::locate(uchar *str, uint strLen)
{
...
uchar *
StringDictionaryMMPFC::extract(size_t id, uint *strLen)
{
...
//	}
	...
	while (1) {
		...
		if (maxlcp < offset ) {
			...
			for (i = maxlcp; i < offset && this->textStrings[basepos+i-maxlcp]; i++) {
...
IteratorDictID*
StringDictionaryMMPFC::locatePrefix(uchar *str, uint strLen)
{
...
IteratorDictID*
StringDictionaryMMPFC::locateSubstr(uchar *str, uint strLen)
{
...
uint 
StringDictionaryMMPFC::locateRank(uint rank)
{
	return rank;
}

IteratorDictString*
StringDictionaryMMPFC::extractPrefix(uchar *str, uint strLen)
{
...
IteratorDictString*
StringDictionaryMMPFC::extractTable()
{
...
IteratorDictString*
StringDictionaryMMPFC::extractSubstr(uchar *str, uint strLen)
{
...
uchar *
StringDictionaryMMPFC::extractRank(uint rank, uint *strLen)
{
	return extract(rank, strLen);
}

size_t 
StringDictionaryMMPFC::getSize()
{
	return (bytesStrings*sizeof(uchar))+this->selPointers->getSize()+llcp->getSize()+rlcp->getSize()+sizeof(StringDictionaryMMPFC);
}

void 
StringDictionaryMMPFC::save(ofstream &out)
{
...
StringDictionary*
StringDictionaryMMPFC::load(ifstream &in)
{
	size_t type = loadValue<uint32_t>(in);
	if(type != MMPFC) return NULL;

	cout << "Reading MMPFC" <<endl;

	StringDictionaryMMPFC *dict = new StringDictionaryMMPFC();
...
//void
//StringDictionaryMMPFC::decodeNextString(uchar **ptr, uint lenPrefix, uchar *str, uint *strLen)
//{
//	uint lenSuffix;
//
...
//uint
//StringDictionaryMMPFC::searchPrefix(uchar **ptr, uint scanneable, uchar *decoded, uint *decLen, uchar *str, uint strLen)
//{
...
//
//uint
//StringDictionaryMMPFC::searchDistinctPrefix(uchar *ptr, uint scanneable, uchar *decoded, uint *decLen, uchar *str, uint strLen)
//{
//	uint id = 1;
//	uint lenPrefix;
//
...
void StringDictionaryMMPFC::printInfo() {
...
void StringDictionaryMMPFC::buildLcps(uchar *dict, uint *pointers, vector<size_t>*llcp, vector<size_t>*rlcp, uint l, uint r) {
    uint m = (l + r) / 2;
//    uint level = bits(r-l);
//    if (level > 10)
...
inline int StringDictionaryMMPFC::doSearchString(uchar * string, uint len, uint lpos, uint rpos, uint l, uint r) {
    uint m = (lpos + rpos) / 2;
    uint foo;
...
//    printf("%s, %d %d %d %d %d\n", string, len, lpos, rpos, l, r);
    ...
    if (l > r || (l==r && chooseLeft) ) {
        ...
        if (llcpvalue > l) {
        	...
        	uchar * dictm = this->textStrings + this->selPointers->select1(m+1);
        	uint p = l;
...
inline int StringDictionaryMMPFC::compareNewPattern(uchar *pattern, uchar *compared, uint *pos) {
	uint p=*pos;
