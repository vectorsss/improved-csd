...
StringDictionaryMMRPFCbSD::StringDictionaryMMRPFCbSD()
{
	this->type = MMRPFCbSD;
	this->elements = 0;
	this->maxlength = 0;
...
StringDictionaryMMRPFCbSD::StringDictionaryMMRPFCbSD(IteratorDictString *it)
{
...
//	Block to actually remove characters unneded because of Front-Coding
	...
	//printf("indice = %d, io = %d, ic = %d, intStrings.size = %d\n", indice, io, ic, intStrings.size());
...
inline uint
StringDictionaryMMRPFCbSD::decodeString(uchar *str, uchar *ptr, uint limitstring, uint limitbytes)
{
...
uint 
StringDictionaryMMRPFCbSD::locate(uchar *str, uint strLen)
{
...
uchar *
StringDictionaryMMRPFCbSD::extract(size_t id, uint *strLen)
{
...
IteratorDictID*
StringDictionaryMMRPFCbSD::locatePrefix(uchar *str, uint strLen)
{
...
IteratorDictID*
StringDictionaryMMRPFCbSD::locateSubstr(uchar *str, uint strLen)
{
...
uint 
StringDictionaryMMRPFCbSD::locateRank(uint rank)
{
	return rank;
}

IteratorDictString*
StringDictionaryMMRPFCbSD::extractPrefix(uchar *str, uint strLen)
{
...
IteratorDictString*
StringDictionaryMMRPFCbSD::extractTable()
{
...
IteratorDictString*
StringDictionaryMMRPFCbSD::extractSubstr(uchar *str, uint strLen)
{
...
uchar *
StringDictionaryMMRPFCbSD::extractRank(uint rank, uint *strLen)
{
	return extract(rank, strLen);
}

size_t 
StringDictionaryMMRPFCbSD::getSize()
{
	return (bytesStrings*sizeof(uchar))+this->selPointers->getSize()+llcp->getSize()+rlcp->getSize()+sizeof(StringDictionaryMMRPFCbSD)+rp->getSize();
}

void 
StringDictionaryMMRPFCbSD::save(ofstream &out)
{
...
StringDictionary*
StringDictionaryMMRPFCbSD::load(ifstream &in)
{
	size_t type = loadValue<uint32_t>(in);
	if(type != MMRPFCbSD) return NULL;

	cout << "Reading MMRPFCbSD" <<endl;

	StringDictionaryMMRPFCbSD *dict = new StringDictionaryMMRPFCbSD();
...
//void
//StringDictionaryMMRPFCbSD::decodeNextString(uchar **ptr, uint lenPrefix, uchar *str, uint *strLen)
//{
//	uint lenSuffix;
//
...
//uint
//StringDictionaryMMRPFCbSD::searchPrefix(uchar **ptr, uint scanneable, uchar *decoded, uint *decLen, uchar *str, uint strLen)
//{
...
//
//uint
//StringDictionaryMMRPFCbSD::searchDistinctPrefix(uchar *ptr, uint scanneable, uchar *decoded, uint *decLen, uchar *str, uint strLen)
//{
//	uint id = 1;
//	uint lenPrefix;
//
...
void StringDictionaryMMRPFCbSD::printInfo() {
...
//    printf("XBWInt -> %lu\n", this->xbw->getSize());
//    printf("----------------------------\n");
    printf("MMPFC structure: %lu\n", sizeof(StringDictionaryMMRPFCbSD));
    printf("TOTAL  -> %lu\n", this->getSize());
//    printf("****************************\n");
//    printf("\n\n");
//
...
void StringDictionaryMMRPFCbSD::buildLcps(uchar *dict, uint *pointers, vector<size_t>*llcp, vector<size_t>*rlcp, uint l, uint r) {
    uint m = (l + r) / 2;
//    uint level = bits(r-l);
//    if (level > 10)
...
/*
inline uint
StringDictionaryMMRPFCbSD::decodeCompareString(uchar *ptr, uchar *pattern, uint *pos) 
{
...
inline uint
StringDictionaryMMRPFCbSD::decodeCompareString(uchar *ptr, uchar *pattern, uint *pos, int len) 
{
...
inline int 
StringDictionaryMMRPFCbSD::doSearchString(uchar * string, uint len, uint lpos, uint rpos, uint l, uint r) {
