...
StringDictionaryMMPFCbSD::StringDictionaryMMPFCbSD()
{
	this->type = MMPFCbSD;
	this->elements = 0;
	this->maxlength = 0;
...
StringDictionaryMMPFCbSD::StringDictionaryMMPFCbSD(IteratorDictString *it)
{
...
uint 
StringDictionaryMMPFCbSD::locate(uchar *str, uint strLen)
{
...
uchar *
StringDictionaryMMPFCbSD::extract(size_t id, uint *strLen)
{
...
IteratorDictID*
StringDictionaryMMPFCbSD::locatePrefix(uchar *str, uint strLen)
{
...
IteratorDictID*
StringDictionaryMMPFCbSD::locateSubstr(uchar *str, uint strLen)
{
...
uint 
StringDictionaryMMPFCbSD::locateRank(uint rank)
{
	return rank;
}

IteratorDictString*
StringDictionaryMMPFCbSD::extractPrefix(uchar *str, uint strLen)
{
...
IteratorDictString*
StringDictionaryMMPFCbSD::extractTable()
{
...
IteratorDictString*
StringDictionaryMMPFCbSD::extractSubstr(uchar *str, uint strLen)
{
...
uchar *
StringDictionaryMMPFCbSD::extractRank(uint rank, uint *strLen)
{
	return extract(rank, strLen);
}

size_t 
StringDictionaryMMPFCbSD::getSize()
{
	return (bytesStrings*sizeof(uchar))+this->selPointers->getSize()+llcp->getSize()+rlcp->getSize()+sizeof(StringDictionaryMMPFCbSD);
}

void 
StringDictionaryMMPFCbSD::save(ofstream &out)
{
...
StringDictionary*
StringDictionaryMMPFCbSD::load(ifstream &in)
{
	size_t type = loadValue<uint32_t>(in);
	if(type != MMPFCbSD) return NULL;

	cout << "Reading MMPFCbSD" <<endl;

	StringDictionaryMMPFCbSD *dict = new StringDictionaryMMPFCbSD();
...
//void
//StringDictionaryMMPFCbSD::decodeNextString(uchar **ptr, uint lenPrefix, uchar *str, uint *strLen)
//{
//	uint lenSuffix;
//
...
//uint
//StringDictionaryMMPFCbSD::searchPrefix(uchar **ptr, uint scanneable, uchar *decoded, uint *decLen, uchar *str, uint strLen)
//{
...
//
//uint
//StringDictionaryMMPFCbSD::searchDistinctPrefix(uchar *ptr, uint scanneable, uchar *decoded, uint *decLen, uchar *str, uint strLen)
//{
//	uint id = 1;
//	uint lenPrefix;
//
...
void StringDictionaryMMPFCbSD::printInfo() {
...
//    printf("XBWInt -> %lu\n", this->xbw->getSize());
//    printf("----------------------------\n");
    printf("MMPFC structure: %lu\n", sizeof(StringDictionaryMMPFCbSD));
    printf("TOTAL  -> %lu\n", this->getSize());
//    printf("****************************\n");
//    printf("\n\n");
//
...
void StringDictionaryMMPFCbSD::buildLcps(uchar *dict, uint *pointers, vector<size_t>*llcp, vector<size_t>*rlcp, uint l, uint r) {
    uint m = (l + r) / 2;
//    uint level = bits(r-l);
//    if (level > 10)
...
inline int StringDictionaryMMPFCbSD::doSearchString(uchar * string, uint len, uint lpos, uint rpos, uint l, uint r) {
    uint m = (lpos + rpos) / 2;
    uint foo;
...
inline int StringDictionaryMMPFCbSD::compareNewPattern(uchar *pattern, uchar *compared, uint *pos, int len) {
	uint p=*pos;
...
//	printf("got pos=%d, pattern=%d, compared=%d\n", p, pattern[p], compared[p]);
	*pos = p;
	if (p == len) {
		return pattern[p];
	}
