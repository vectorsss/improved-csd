...
CRePair::compress_pair_table() 
{
	...
	for (i =0 ; i< (max_assigned-max_value) ; i++)
	{
		if (symbols_pair_tmp[i] == 0) 
		{
			...
			cds_utils::bit_set(symbols_new_bit,j-1); j++;
...
CRePair::fillBR() 
{
	...
	pos = 0;
	for(uint i=0;i<m;i++) { cds_utils::bit_set(bits_BR,pos); unroll(symbols[i],i); }
	cds_utils::bit_set(bits_BR,pos);
...
CRePair::new_value(uint *symbols_pair, uint *symbols_new_value, uint *k1, uint *j, uint pos) 
{
	...
	if (izq>max_value) 
	{
		...
		if (symbols_new_value[izq] == 0) 
		{
			...
			cds_utils::bit_set(symbols_new_bit,*j-1);
			(*j)++;
