...
public:
	// creates empty array
	static TarrayCSD createArray (void *Rec, float factor, int minsize);
	// destroys A
	static void destroyArray (TarrayCSD *A);

	// inserts pair in A, returns pos
	static int insertArray (TarrayCSD *A, int pair);
	 // deletes last cell in A
	static void deleteArray (TarrayCSD *A);
};
