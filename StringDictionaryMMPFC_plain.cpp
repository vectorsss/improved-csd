...
StringDictionaryMMPFCPLAIN::StringDictionaryMMPFCPLAIN()
{
	this->type = MMPFCPLAIN;
	this->elements = 0;
	this->maxlength = 0;
}

StringDictionaryMMPFCPLAIN::StringDictionaryMMPFCPLAIN(IteratorDictString *it)
{
...
uint 
StringDictionaryMMPFCPLAIN::locate(uchar *str, uint strLen)
{
...
uchar *
StringDictionaryMMPFCPLAIN::extract(size_t id, uint *strLen)
{
...
IteratorDictID*
StringDictionaryMMPFCPLAIN::locatePrefix(uchar *str, uint strLen)
{
...
IteratorDictID*
StringDictionaryMMPFCPLAIN::locateSubstr(uchar *str, uint strLen)
{
...
uint 
StringDictionaryMMPFCPLAIN::locateRank(uint rank)
{
	return rank;
}

IteratorDictString*
StringDictionaryMMPFCPLAIN::extractPrefix(uchar *str, uint strLen)
{
...
IteratorDictString*
StringDictionaryMMPFCPLAIN::extractTable()
{
...
IteratorDictString*
StringDictionaryMMPFCPLAIN::extractSubstr(uchar *str, uint strLen)
{
...
uchar *
StringDictionaryMMPFCPLAIN::extractRank(uint rank, uint *strLen)
{
	return extract(rank, strLen);
}

size_t 
StringDictionaryMMPFCPLAIN::getSize()
{
	return (bytesStrings*sizeof(uchar))+(this->realElements+1)*sizeof(uint)+llcp->getSize()+rlcp->getSize()+sizeof(StringDictionaryMMPFCPLAIN);
}

void 
StringDictionaryMMPFCPLAIN::save(ofstream &out)
{
...
StringDictionary*
StringDictionaryMMPFCPLAIN::load(ifstream &in)
{
	size_t type = loadValue<uint32_t>(in);
	if(type != MMPFCPLAIN) return NULL;

	cout << "Reading MMPFCPLAIN" <<endl;

	StringDictionaryMMPFCPLAIN *dict = new StringDictionaryMMPFCPLAIN();

	dict->type = MMPFCPLAIN;
...
//void
//StringDictionaryMMPFCPLAIN::decodeNextString(uchar **ptr, uint lenPrefix, uchar *str, uint *strLen)
//{
//	uint lenSuffix;
//
...
//uint
//StringDictionaryMMPFCPLAIN::searchPrefix(uchar **ptr, uint scanneable, uchar *decoded, uint *decLen, uchar *str, uint strLen)
//{
...
//
//uint
//StringDictionaryMMPFCPLAIN::searchDistinctPrefix(uchar *ptr, uint scanneable, uchar *decoded, uint *decLen, uchar *str, uint strLen)
//{
//	uint id = 1;
//	uint lenPrefix;
//
...
void StringDictionaryMMPFCPLAIN::printInfo() {
...
//    printf("XBWInt -> %lu\n", this->xbw->getSize());
//    printf("----------------------------\n");
    printf("MMPFC structure: %lu\n", sizeof(StringDictionaryMMPFCPLAIN));
    printf("TOTAL  -> %lu\n", this->getSize());
//    printf("****************************\n");
//    printf("\n\n");
//
...
void StringDictionaryMMPFCPLAIN::buildLcps(uchar *dict, uint *pointers, vector<size_t>*llcp, vector<size_t>*rlcp, uint l, uint r) {
    uint m = (l + r) / 2;
//    uint level = bits(r-l);
//    if (level > 10)
...
inline int StringDictionaryMMPFCPLAIN::doSearchString(uchar * string, uint len, uint lpos, uint rpos, uint l, uint r) {
    uint m = (lpos + rpos) / 2;
    uint foo;

//    printf("%s, %d %d %d %d %d %d\n", string, len, lpos, rpos, l, r, m);
...
inline int StringDictionaryMMPFCPLAIN::compareNewPattern(uchar *pattern, uchar *compared, uint *pos, int len) {
	uint p=*pos;
