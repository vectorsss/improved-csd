...
StringDictionaryMMPFCbSDRlcp::StringDictionaryMMPFCbSDRlcp()
{
	this->type = MMPFCbSDRLCP;
	this->elements = 0;
	this->maxlength = 0;
}

StringDictionaryMMPFCbSDRlcp::StringDictionaryMMPFCbSDRlcp(IteratorDictString *it)
{
...
uint 
StringDictionaryMMPFCbSDRlcp::locate(uchar *str, uint strLen)
{
...
uchar *
StringDictionaryMMPFCbSDRlcp::extract(size_t id, uint *strLen)
{
...
IteratorDictID*
StringDictionaryMMPFCbSDRlcp::locatePrefix(uchar *str, uint strLen)
{
...
IteratorDictID*
StringDictionaryMMPFCbSDRlcp::locateSubstr(uchar *str, uint strLen)
{
...
uint 
StringDictionaryMMPFCbSDRlcp::locateRank(uint rank)
{
	return rank;
}

IteratorDictString*
StringDictionaryMMPFCbSDRlcp::extractPrefix(uchar *str, uint strLen)
{
...
IteratorDictString*
StringDictionaryMMPFCbSDRlcp::extractTable()
{
...
IteratorDictString*
StringDictionaryMMPFCbSDRlcp::extractSubstr(uchar *str, uint strLen)
{
...
uchar *
StringDictionaryMMPFCbSDRlcp::extractRank(uint rank, uint *strLen)
{
	return extract(rank, strLen);
}

size_t 
StringDictionaryMMPFCbSDRlcp::getSize()
{
	return (bytesStrings*sizeof(uchar))+this->selPointers->getSize()+rlcp->getSize()+sizeof(StringDictionaryMMPFCbSDRlcp);
}

void 
StringDictionaryMMPFCbSDRlcp::save(ofstream &out)
{
...
StringDictionary*
StringDictionaryMMPFCbSDRlcp::load(ifstream &in)
{
	size_t type = loadValue<uint32_t>(in);
	if(type != MMPFCbSDRLCP) return NULL;

	cout << "Reading MMPFCbSDRLCP" <<endl;

	StringDictionaryMMPFCbSDRlcp *dict = new StringDictionaryMMPFCbSDRlcp();

	dict->type = MMPFCbSDRLCP;
...
//void
//StringDictionaryMMPFCbSDRlcp::decodeNextString(uchar **ptr, uint lenPrefix, uchar *str, uint *strLen)
//{
//	uint lenSuffix;
//
...
//uint
//StringDictionaryMMPFCbSDRlcp::searchPrefix(uchar **ptr, uint scanneable, uchar *decoded, uint *decLen, uchar *str, uint strLen)
//{
...
//
//uint
//StringDictionaryMMPFCbSDRlcp::searchDistinctPrefix(uchar *ptr, uint scanneable, uchar *decoded, uint *decLen, uchar *str, uint strLen)
//{
//	uint id = 1;
//	uint lenPrefix;
//
...
void StringDictionaryMMPFCbSDRlcp::printInfo() {
...
//    printf("XBWInt -> %lu\n", this->xbw->getSize());
//    printf("----------------------------\n");
    printf("MMPFC structure: %lu\n", sizeof(StringDictionaryMMPFCbSDRlcp));
    printf("TOTAL  -> %lu\n", this->getSize());
//    printf("****************************\n");
//    printf("\n\n");
//
...
void StringDictionaryMMPFCbSDRlcp::buildLcps(uchar *dict, uint *pointers, vector<size_t>*llcp, vector<size_t>*rlcp, uint l, uint r) {
    uint m = (l + r) / 2;
//    uint level = bits(r-l);
//    if (level > 10)
...
inline int StringDictionaryMMPFCbSDRlcp::doSearchString(uchar * string, uint len, uint lpos, uint rpos, uint l, uint r) {
    uint m = (lpos + rpos) / 2;
    uint foo;
...
inline int StringDictionaryMMPFCbSDRlcp::compareNewPattern(uchar *pattern, uchar *compared, uint *pos, int len) {
	uint p=*pos;
