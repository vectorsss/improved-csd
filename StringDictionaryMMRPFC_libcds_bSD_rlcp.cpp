...
StringDictionaryMMRPFCbSDRlcp::StringDictionaryMMRPFCbSDRlcp()
{
	this->type = MMRPFCbSDRLCP;
	this->elements = 0;
	this->maxlength = 0;
}

StringDictionaryMMRPFCbSDRlcp::StringDictionaryMMRPFCbSDRlcp(IteratorDictString *it)
{
...
inline uint
StringDictionaryMMRPFCbSDRlcp::decodeString(uchar *str, uchar *ptr, uint limitstring, uint limitbytes)
{
...
uint 
StringDictionaryMMRPFCbSDRlcp::locate(uchar *str, uint strLen)
{
...
uchar *
StringDictionaryMMRPFCbSDRlcp::extract(size_t id, uint *strLen)
{
...
IteratorDictID*
StringDictionaryMMRPFCbSDRlcp::locatePrefix(uchar *str, uint strLen)
{
...
IteratorDictID*
StringDictionaryMMRPFCbSDRlcp::locateSubstr(uchar *str, uint strLen)
{
...
uint 
StringDictionaryMMRPFCbSDRlcp::locateRank(uint rank)
{
	return rank;
}

IteratorDictString*
StringDictionaryMMRPFCbSDRlcp::extractPrefix(uchar *str, uint strLen)
{
...
IteratorDictString*
StringDictionaryMMRPFCbSDRlcp::extractTable()
{
...
IteratorDictString*
StringDictionaryMMRPFCbSDRlcp::extractSubstr(uchar *str, uint strLen)
{
...
uchar *
StringDictionaryMMRPFCbSDRlcp::extractRank(uint rank, uint *strLen)
{
	return extract(rank, strLen);
}

size_t 
StringDictionaryMMRPFCbSDRlcp::getSize()
{
	return (bytesStrings*sizeof(uchar))+this->selPointers->getSize()+rlcp->getSize()+sizeof(StringDictionaryMMRPFCbSDRlcp)+rp->getSize();
}

void 
StringDictionaryMMRPFCbSDRlcp::save(ofstream &out)
{
...
StringDictionary*
StringDictionaryMMRPFCbSDRlcp::load(ifstream &in)
{
	size_t type = loadValue<uint32_t>(in);
	if(type != MMRPFCbSDRLCP) return NULL;

	cout << "Reading MMRPFCbSDRLCP" <<endl;

	StringDictionaryMMRPFCbSDRlcp *dict = new StringDictionaryMMRPFCbSDRlcp();

	dict->type = MMRPFCbSDRLCP;
...
//void
//StringDictionaryMMRPFCbSDRlcp::decodeNextString(uchar **ptr, uint lenPrefix, uchar *str, uint *strLen)
//{
//	uint lenSuffix;
//
...
//uint
//StringDictionaryMMRPFCbSDRlcp::searchPrefix(uchar **ptr, uint scanneable, uchar *decoded, uint *decLen, uchar *str, uint strLen)
//{
...
//
//uint
//StringDictionaryMMRPFCbSDRlcp::searchDistinctPrefix(uchar *ptr, uint scanneable, uchar *decoded, uint *decLen, uchar *str, uint strLen)
//{
//	uint id = 1;
//	uint lenPrefix;
//
...
void StringDictionaryMMRPFCbSDRlcp::printInfo() {
...
//    printf("XBWInt -> %lu\n", this->xbw->getSize());
//    printf("----------------------------\n");
    printf("MMPFC structure: %lu\n", sizeof(StringDictionaryMMRPFCbSDRlcp));
    printf("TOTAL  -> %lu\n", this->getSize());
//    printf("****************************\n");
//    printf("\n\n");
//
...
void StringDictionaryMMRPFCbSDRlcp::buildLcps(uchar *dict, uint *pointers, vector<size_t>*llcp, vector<size_t>*rlcp, uint l, uint r) {
    uint m = (l + r) / 2;
//    uint level = bits(r-l);
//    if (level > 10)
...
/*
inline uint
StringDictionaryMMRPFCbSDRlcp::decodeCompareString(uchar *ptr, uchar *pattern, uint *pos) 
{
...
inline uint
StringDictionaryMMRPFCbSDRlcp::decodeCompareString(uchar *ptr, uchar *pattern, uint *pos, int len) 
{
...
inline int 
StringDictionaryMMRPFCbSDRlcp::doSearchString(uchar * string, uint len, uint lpos, uint rpos, uint l, uint r) {
