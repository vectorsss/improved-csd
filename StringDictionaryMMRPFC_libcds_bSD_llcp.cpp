...
StringDictionaryMMRPFCbSDLlcp::StringDictionaryMMRPFCbSDLlcp()
{
	this->type = MMRPFCbSDLLCP;
	this->elements = 0;
	this->maxlength = 0;
}

StringDictionaryMMRPFCbSDLlcp::StringDictionaryMMRPFCbSDLlcp(IteratorDictString *it)
{
...
//	Block to actually remove characters unneded because of Front-Coding
	...
	this->llcp = new LogSequence(&llcp, bits(maxlength));
		...
		/* DESCARTADA: la alternativa de uso de DAC para los valores de los lcps no ahorra lo suficiente
		para compensar el empeoramiento de tiempos. En la versión diff se probará a usar codficación diferencial */
		/*
		...
		fprintf(stderr, "\tdacllcp SIZE!!!! %d vs. LogSequence %d\n", dacllcp->getSize(), this->llcp->getSize());
		fprintf(stderr, "\tdacrlcp SIZE!!!! %d vs. LogSequence %d\n", dacrlcp->getSize(), this->rlcp->getSize());
		*/
...
inline uint
StringDictionaryMMRPFCbSDLlcp::decodeString(uchar *str, uchar *ptr, uint limitstring, uint limitbytes)
{
...
uint 
StringDictionaryMMRPFCbSDLlcp::locate(uchar *str, uint strLen)
{
...
uchar *
StringDictionaryMMRPFCbSDLlcp::extract(size_t id, uint *strLen)
{
...
IteratorDictID*
StringDictionaryMMRPFCbSDLlcp::locatePrefix(uchar *str, uint strLen)
{
...
IteratorDictID*
StringDictionaryMMRPFCbSDLlcp::locateSubstr(uchar *str, uint strLen)
{
...
uint 
StringDictionaryMMRPFCbSDLlcp::locateRank(uint rank)
{
	return rank;
}

IteratorDictString*
StringDictionaryMMRPFCbSDLlcp::extractPrefix(uchar *str, uint strLen)
{
...
IteratorDictString*
StringDictionaryMMRPFCbSDLlcp::extractTable()
{
...
IteratorDictString*
StringDictionaryMMRPFCbSDLlcp::extractSubstr(uchar *str, uint strLen)
{
...
uchar *
StringDictionaryMMRPFCbSDLlcp::extractRank(uint rank, uint *strLen)
{
	return extract(rank, strLen);
}

size_t 
StringDictionaryMMRPFCbSDLlcp::getSize()
{
	return (bytesStrings*sizeof(uchar))+this->selPointers->getSize()+llcp->getSize()+sizeof(StringDictionaryMMRPFCbSDLlcp)+rp->getSize();
}

void 
StringDictionaryMMRPFCbSDLlcp::save(ofstream &out)
{
...
StringDictionary*
StringDictionaryMMRPFCbSDLlcp::load(ifstream &in)
{
	size_t type = loadValue<uint32_t>(in);
	if(type != MMRPFCbSDLLCP) return NULL;

	cout << "Reading MMRPFCbSDLLCP" <<endl;

	StringDictionaryMMRPFCbSDLlcp *dict = new StringDictionaryMMRPFCbSDLlcp();

	dict->type = MMRPFCbSDLLCP;
...
//void
//StringDictionaryMMRPFCbSDLlcp::decodeNextString(uchar **ptr, uint lenPrefix, uchar *str, uint *strLen)
//{
//	uint lenSuffix;
//
...
//uint
//StringDictionaryMMRPFCbSDLlcp::searchPrefix(uchar **ptr, uint scanneable, uchar *decoded, uint *decLen, uchar *str, uint strLen)
//{
...
//
//uint
//StringDictionaryMMRPFCbSDLlcp::searchDistinctPrefix(uchar *ptr, uint scanneable, uchar *decoded, uint *decLen, uchar *str, uint strLen)
//{
//	uint id = 1;
//	uint lenPrefix;
//
...
void StringDictionaryMMRPFCbSDLlcp::printInfo() {
    ...
    printf("repair + Vbyte: %lu\n", this->rp->getSize());
//    printf("XBWInt -> %lu\n", this->xbw->getSize());
//    printf("----------------------------\n");
    printf("MMPFC structure: %lu\n", sizeof(StringDictionaryMMRPFCbSDLlcp));
    printf("TOTAL  -> %lu\n", this->getSize());
//    printf("****************************\n");
//    printf("\n\n");
//
...
void StringDictionaryMMRPFCbSDLlcp::buildLcps(uchar *dict, uint *pointers, vector<size_t>*llcp, vector<size_t>*rlcp, uint l, uint r) {
    uint m = (l + r) / 2;
//    uint level = bits(r-l);
//    if (level > 10)
...
/*
inline uint
StringDictionaryMMRPFCbSDLlcp::decodeCompareString(uchar *ptr, uchar *pattern, uint *pos) 
{
...
inline uint
StringDictionaryMMRPFCbSDLlcp::decodeCompareString(uchar *ptr, uchar *pattern, uint *pos, int len) 
{
...
//		printf("igual hasta posición %d\n", posCmp);
	...
	*pos = posCmp;
//	printf("bytes %d, len = %d, pattern[%d]=%d, cmp = %d. ptr=%d\n", bytes, len, posCmp, pattern[posCmp], cmp, *ptr);
	return (bytes == len) ? pattern[posCmp]: ((*ptr == 128)?0:-1);
}

inline int 
StringDictionaryMMRPFCbSDLlcp::doSearchString(uchar * string, uint len, uint lpos, uint rpos, uint l, uint r) {
