...
bool TestBitSequence(const BitSequence &ref, const BitSequence &bs){
    ulong nones = ref.rank1(ref.getLength()-1);
    assert(ref.getLength() == bs.getLength());
    for (ulong i=0;i<ref.getLength();i++){
        assert(ref.access(i)==bs.access(i));
    }
    cout << "Access OK" << endl;
    for (ulong i=0;i<ref.getLength();i++){
        assert(ref.rank0(i)==ref.rank0(i));
    }
    cout << "Rank0 OK" << endl;
    for (ulong i=0;i<ref.getLength();i++){
        assert(ref.rank1(i)==ref.rank1(i));
    }
    cout << "Rank1 OK" << endl;
    for (ulong i=0;i<nones;i++){
        assert(ref.select1(i+1)==ref.select1(i+1));
    }
    cout << "Select1 OK" << endl;
    ulong nzeros = ref.getLength()-nones;
    for (ulong i=0;i<nzeros;i++){
        assert(ref.select0(i+1)==ref.select0(i+1));
    }
...
// BitSequence * s;
  ...
  BitSequence * s;
    s = new BitSequenceDelta(data,len,33);
    BitSequence *s2 = new BitSequenceRG(data,len,33);
    TestBitSequence(*s2,*s);
