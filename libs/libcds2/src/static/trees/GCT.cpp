...
GCT::GCT(uint _delta,  uint _samplingC, uint *input, char *inputGrammar):Parentheses(0) {
	...
	ifstream iff(inputGrammar);
		uint hdr = loadValue<uint>(iff);
	len = loadValue<uint>(iff);
	n = loadValue<uint>(iff);
	alph = loadValue<uint>(iff);
	c = loadValue<uint>(iff);
	RULES_LEFT = loadValue<uint>(iff,n-alph);
	RULES_RIGHT = loadValue<uint>(iff,n-alph);
	rule_lengths = loadValue<uint>(iff,n-alph);
	newC = loadValue<uint>(iff,c);
	iff.close();
	delta=_delta;	
...
void GCT::build(uint *input,ulong len){
	...
	uint *PERM;
	dic = new DictionaryRP(RULES_LEFT,RULES_RIGHT,new_alph,newn,PERM);
