...
namespace cds_static{
    ...
    */
	...
	BP::BP(uint *input, size_t len, bool reversed, bool deleted){
		...
		for (size_t i=0;i<nb;i++){
			...
			for (uint j=0;j<lim;j++){
				if (bitget(input+i,j)){
					bit_set(new_input + i, j);
				}
				if (bitget(input+i,31-j)){
					bit_set(new_input + i, 31 - j);
				}
			}
		}
		if (reversed){
	...
	ulong BP::getSize() const{
		return (ulong) (internal_bp->idx_size+n/8.0);
	}
	...
	ulong BP::bwd_search(uint v, int e){
		return (ulong) bp_bwd_excess(internal_bp,v,e);
	}
	ulong BP::fwdExcess(uint s, uint e){
		return (ulong) bp_fwd_excess(internal_bp,s,e);
	}
	ulong BP::bwdExcess( uint s, int e){
		return (ulong) bp_bwd_excess(internal_bp,s,e);
	}
	ulong BP::rmq( uint s, int t, int &m){
		m = -1;
		return (ulong) bp_rmq(internal_bp, s, t, 0);
	}
	ulong BP::depth( uint s){
		return (ulong) bp_depth(internal_bp, s);
	}
	ulong BP::rankOpen( uint s){
		return (ulong) bp_rank_open(internal_bp,s);
	}
	ulong BP::rankClose( uint s){
		return (ulong) bp_rank_close(internal_bp, s);
	}
	ulong BP::selectOpen( uint s){
		return (ulong) bp_select_open(internal_bp, s);
	}
	ulong BP::selectClose( uint s){
		return (ulong) bp_select_close(internal_bp, s);
	}
	ulong BP::root_node(){
		return (ulong) 0;
	}
	ulong BP::inspect(uint s){
		return (ulong) bp_inspect(internal_bp, s);
	}
	ulong BP::findClose(uint s){
		return (ulong) bp_find_close(internal_bp, s);
	}
	ulong BP::findOpen(uint s){
		return (ulong) bp_find_open(internal_bp, s);
	}
	ulong BP::parent(uint s){
		return (ulong) bp_parent(internal_bp, s);
	}
	ulong BP::parentClose(uint s){
		return (ulong) bp_parent_close(internal_bp, s);
	}
	ulong BP::enclose(uint s){
		return (ulong) bp_enclose(internal_bp, s);
	}
	ulong BP::levelAncestor(uint s, uint d){
		return (ulong) bp_level_ancestor(internal_bp, s, d);
	}
	ulong BP::lca(uint a, uint b){
		return (ulong) bp_lca(internal_bp, a, b);
	}
	ulong BP::preorderRank(uint s){
		return (ulong) bp_preorder_rank(internal_bp, s);
	}
	ulong BP::preorderSelect(uint s){
		return (ulong) bp_preorder_select(internal_bp,s);
	}
	ulong BP::postorderRank(uint s){
		return (ulong) bp_postorder_rank(internal_bp, s);
	}
	ulong BP::isLeaf(uint s){
		return (ulong) bp_isleaf(internal_bp, s);
	}
	ulong BP::subtreeSize(uint s){
		return (ulong) bp_subtree_size(internal_bp, s);
	}
	ulong BP::firstChild(uint s){
		return (ulong) bp_first_child(internal_bp, s);
	}
	ulong BP::nextSibling(uint s){
		return (ulong) bp_next_sibling(internal_bp, s);
	}
	ulong BP::isAncestor(uint a, uint b){
		return (ulong) bp_is_ancestor(internal_bp, a,b);
	}
	ulong BP::degree(uint x){
		return (ulong) bp_degree(internal_bp, x);
	}
	ulong BP::childRank(uint x){
		return (ulong) bp_child_rank(internal_bp, x);
	}
	ulong BP::child(uint x, uint i){
		return (ulong) bp_child(internal_bp, x, i);
	}
	ulong BP::prevSibling(uint x){
		return (ulong) bp_prev_sibling(internal_bp, x);
	}
}
