...
namespace cds_static
{
	...
	BitSequenceRPSC::BitSequenceRPSC(uint * bitseq, size_t n, BitSequenceBuilder *bsb, PermutationBuilder *pb, uint samplingC, uint delta, uint ss_rate, bool deleteSymbols) {
		length = n;
		...
		seq= new SequenceRepairSC(new_seq,n,bsb, new MapperNone(),pb,samplingC,delta,ss_rate,deleteSymbols);
		if (new_seq) delete [] new_seq;
	}

	BitSequenceRPSC::BitSequenceRPSC(uint * bitseq, size_t n, Repair *rp, BitSequenceBuilder *bsb, PermutationBuilder *pb, uint samplingC, uint delta, uint ss_rate, bool deleteSymbols) {
		length = n;
		uint *new_seq = new uint[n];
		for (size_t i=0;i<n;i++){
			if (bitget(bitseq,i)){
		...
		seq= new SequenceRepairSC(new_seq,n, rp, bsb, new MapperNone(),pb,samplingC,delta,ss_rate,deleteSymbols);
		if (new_seq) delete [] new_seq;
	}
