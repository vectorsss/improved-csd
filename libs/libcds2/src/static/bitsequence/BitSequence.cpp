...
namespace cds_static
{
	...
	BitSequence* BitSequence::getFromBitSequence(BitSequence &bs, BitSequenceBuilder &builder){
		uint *bmp;
		...
		BitSequence * b = builder.build(bmp,len);
		delete [] bmp;
		return b;
	}
	...
	BitSequence * BitSequence::load(ifstream & fp) {
		...
		switch(r) {
			...
			case PLAIN_HDR: return BitSequencePlain::load(fp); 
			case RP_SC_HDR: return BitSequenceRPSC::load(fp); 
			case RP_SN_HDR: return BitSequenceRPSN::load(fp); 			
			case DELTA: return BitSequenceDelta::load(fp);
		}
		return NULL;
	}
};
