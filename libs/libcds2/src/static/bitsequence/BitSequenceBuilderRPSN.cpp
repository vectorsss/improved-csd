...
namespace cds_static
{
	...
	BitSequenceBuilderRPSN::BitSequenceBuilderRPSN(Repair *rp, BitSequenceBuilder * bsb, PermutationBuilder *pb, uint samplingC, uint delta, uint _ss_rate) {
		this->bsb = bsb;
		this->pb = pb;
	...
	BitSequenceBuilderRPSN::BitSequenceBuilderRPSN(BitSequenceBuilder * bsb, PermutationBuilder *pb, uint samplingC, uint delta, uint _ss_rate) {
		this->rp = nullptr;
		this->bsb = bsb;
		this->pb = pb;
		this->samplingC = samplingC;
	...
	BitSequenceBuilderRPSN::BitSequenceBuilderRPSN(BitSequenceBuilder * bsb, uint samplingC, uint delta, uint _ss_rate) {
		this->rp = nullptr;
		this->bsb = bsb;
	...
	BitSequence * BitSequenceBuilderRPSN::build(uint * bitseq, size_t len) const{
		if (this->rp == nullptr){
			return new BitSequenceRPSN(bitseq,len, bsb, pb, samplingC, delta, ss_rate, true);
		}else{
			return new BitSequenceRPSN(bitseq,len, rp, bsb, pb, samplingC, delta, ss_rate, true);
		}
	}
