...
namespace cds_static
{
	...
	BitSequenceBuilderRPSC::BitSequenceBuilderRPSC(Repair *rp, BitSequenceBuilder * bsb, PermutationBuilder *pb, uint samplingC, uint delta, uint _ss_rate) {
		this->rp = rp;
		this->bsb = bsb;
		this->pb = pb;
	...
	BitSequenceBuilderRPSC::BitSequenceBuilderRPSC(BitSequenceBuilder * bsb, PermutationBuilder *pb, uint samplingC, uint delta, uint _ss_rate) {
		this->rp = nullptr;
		this->bsb = bsb;
		this->pb = pb;
		this->samplingC = samplingC;
	...
	BitSequenceBuilderRPSC::BitSequenceBuilderRPSC(BitSequenceBuilder * bsb, uint samplingC, uint delta, uint _ss_rate) {
		this->bsb = bsb;
	...
	BitSequence * BitSequenceBuilderRPSC::build(uint * bitseq, size_t len) const{
		if (this->rp== nullptr) {
			return new BitSequenceRPSC(bitseq, len, bsb, pb, samplingC, delta, ss_rate);
		}else{
			return new BitSequenceRPSC(bitseq, len, rp, bsb, pb, samplingC, delta, ss_rate);
		}
	}
