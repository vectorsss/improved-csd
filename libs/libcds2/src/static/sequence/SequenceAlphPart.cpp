...
{
	inline uint SequenceAlphPart::group(uint pos, uint cut) const {
		uint ret = 0;
		if(pos<=cut)
			ret = pos;
		else
	...
	inline uint SequenceAlphPart::offset(uint pos, uint cut, uint group) const{
		uint ret = 0;
		if(pos<=cut)
			ret = 0;
		else
	...
	SequenceAlphPart::SequenceAlphPart(size_t len):Sequence(len){};
	...
	SequenceAlphPart::SequenceAlphPart(uint * seq, size_t n, uint _cut, SequenceBuilder * lenIndexBuilder, SequenceBuilder * seqsBuilder) : Sequence(n) {
...
//		delete [] alphSortedByFreq;
	...
	size_t SequenceAlphPart::rank(uint c, size_t i) const
	{
		...
		if(pos<=cut) {
			size_t ret = groupsIndex->rank(pos,i);
			return ret;
		}
	...
	size_t SequenceAlphPart::select(uint c, size_t i) const
	{
		uint pos = revPermFreq[c];
		if(pos<=cut ) {
			size_t ret = groupsIndex->select(pos,i);
			//cerr << "select("<<c<<","<<i<<"): "<<ret << endl;
			return ret;
		}
		uint g = group(pos,cut);
		uint o = offset(pos,cut,g);
	...
	ulong SequenceAlphPart::extract(ulong i, ulong j, vector<uint> &res) const{
		res.reserve(j-i);
		vector<ulong> ranks(maxLen-cut+1,-1UL);
		for (ulong it=i;it<j;it++){
			uint g = groupsIndex->access(it);
			if(g<=cut ){
				res.push_back(alphSortedByFreq[g]);
			}else {
				uint index = g-cut-1;
				if (ranks[index] == -1UL) {
					ranks[index] = groupsIndex->rank(g, it) - 1;
				}
				uint o = indexesByLength[index]->access(ranks[index]++);
				res.push_back(alphSortedByFreq[(1 << (index + bits(cut))) + o]);
			}
		}
		assert(res.size()==(j-i));
		return res.size();
