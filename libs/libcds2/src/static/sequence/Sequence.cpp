...
//			case SEQUENCE_MULTI_ARY_WT_HDR: return MultiArityWT::load(fp);
	...
	void Sequence::buildBestBitmap(vector<BitSequenceBuilder*> &bmb, uint *bmp, size_t len, BitSequence *&bs){
	...
	void Sequence::buildBestSequence(vector<SequenceBuilder*> &bmb, uint *bmp, size_t len, Sequence *&bs){
		bs = bmb[0]->build(bmp,len);
		for (uint j=1;j<bmb.size();j++){
