...
namespace cds_static
{
	...
	Sequence * SequenceBuilderSequenceRepairSC::build(uint * sequence, size_t len) {
		if (rp){
			return new SequenceRepairSC(sequence, len, rp,bsb, am, pb, samplingC, delta, ss_rate);
		}else {
			return new SequenceRepairSC(sequence, len, bsb, am, pb, samplingC, delta, ss_rate);
		}
	}
