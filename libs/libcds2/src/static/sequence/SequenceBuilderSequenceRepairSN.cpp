...
namespace cds_static
{
	...
	SequenceBuilderSequenceRepairSN::SequenceBuilderSequenceRepairSN(BitSequenceBuilder * bsb, 
					Mapper * am,PermutationBuilder *pb, uint samplingC, uint delta, uint _ss_rate) {
		this->rp = nullptr;
		this->bsb = bsb;
		this->am = am;
		this->pb = pb;
	...
	Sequence * SequenceBuilderSequenceRepairSN::build(uint * sequence, size_t len) {
		if (rp){
			return new SequenceRepairSN(sequence,len,rp,bsb, am, pb, samplingC, delta, ss_rate);
		}else {
			return new SequenceRepairSN(sequence, len, bsb, am, pb, samplingC, delta, ss_rate);
		}
	}

	Sequence * SequenceBuilderSequenceRepairSN::build(const Array & seq) {

		return new SequenceRepairSN(seq, bsb, am, pb, samplingC, delta, ss_rate);
