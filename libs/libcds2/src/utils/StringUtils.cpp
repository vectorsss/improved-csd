...
namespace cds_utils
{
	...
	const char *GetFileNameFromPath(const char *filename){
		...
		char end = '\0';
		ulong i=0;
		while(filename[i]!=end){
			if (filename[i]=='/')
				prev_slash=i;
			i++;
		}
		if (filename[prev_slash]=='/')
			prev_slash++;

		return filename+prev_slash;
	}
    ...
    long LoadBitmapVerbatimFromUlongFile(const string &inFile, uint *& input , ulong &n){
        ...
        auto len = (n+sizeof(ulong)*8-1)/(sizeof(ulong)*8);
        ulong *input_l = new ulong[len];
        fp.read((char*)input_l,len*sizeof(long));

        createEmptyBitmap(&input,n);
        for (ulong i=0;i<n;i++){
            if (cds_utils::LibcdsBasicsGeneric::bitget(input_l,i)){
                bit_set(input,i);
            }
        }
        delete [] input_l;
