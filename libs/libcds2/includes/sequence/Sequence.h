...
namespace cds_static
{
	...
	#define ALPHPARTREP_HDR 11
	#define SEQUENCE_REPAIRSN_HDR 12
	#define SEQUENCE_REPAIRSC_HDR 13
	#define SEQUENCE_WT_RP 14
	#define SEQUENCE_WT_NOPTRS_HYBRID 15
	#define SEQUENCE_MATRIX_HDR 16
	#define SEQUENCE_LZEND_HDR 18
	#define SEQUENCE_BLOCK_GRAPH_HDR 19
	...
	class Sequence
	{
		...
		public:
			...
			*/
			...
			/**
			 * Retrieves the symbosl in S[i,min(j,n)) (right open interval)
			 */
		...
		protected:
			size_t length;
			uint sigma;
			void buildBestBitmap(vector<BitSequenceBuilder*> &, uint *bmp, size_t len, BitSequence *&bs);
			void buildBestSequence(vector<SequenceBuilder*> &, uint *bmp, size_t len, Sequence *&bs);
