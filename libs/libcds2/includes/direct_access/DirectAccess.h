...
namespace cds_static{
	...
	class DirectAccess{
		...
		protected:
			...
			size_t len;
        bool allEqual;
			uint * optimizationk(uint * list,size_t listLength, int * nkvalues);
			uint * optimizationk(uint * list,size_t listLength, int * nkvalues, uint Lmax);
			void rec_optim(uint **l, uint **b, long **s, uint t, uint v, uint m, size_t *fc);
	};
};
