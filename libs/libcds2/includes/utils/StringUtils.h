...
//		ml = (long long )modulo;
	...
	template<typename T> long loadFile(const string &inFile, T *& input , long &n, long addSpace=0L) throw(){
		ifstream fp(inFile);
		if (!fp.good()){
		...
		n = len/sizeof(T)+addSpace;
		input = new T[n];
		fp.read((char*)input, len);
		return n;
	}

    template<typename T> long loadFile(const string &inFile, vector<T> &in_v, long addSpace=0L) throw(){
        T *input;long len;
        loadFile<T>(inFile,input,len);
        in_v = vector<T>(input,input+len);
        assert(in_v.size()==len);
        return in_v.size();
    }
	//the input bitmap must be headed by a size_t which tells the length of the bitmap and then the bitmap
	template<typename T> long LoadBitmapVerbatimFromFile(const string &inFile, T *& input , ulong &n) throw(){
		ifstream fp(inFile);
		if (!fp.good()){
			throw new runtime_error("The given input file"+inFile+" is not good.");
		}
		fp.read((char*)&n,sizeof(size_t));
		auto len = (n+sizeof(T)*8-1)/(sizeof(T)*8);;
		input = new T[len];
		fp.read((char*)input,len*sizeof(T));
		return n;
	}

    long LoadBitmapVerbatimFromUlongFile(const string &inFile, uint *& input , ulong &n);
	const char *GetFileNameFromPath(const char *filename);
};
#endif
