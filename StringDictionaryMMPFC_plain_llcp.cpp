...
StringDictionaryMMPFCPLAINLlcp::StringDictionaryMMPFCPLAINLlcp()
{
	this->type = MMPFCPLAINLLCP;
	this->elements = 0;
	this->maxlength = 0;
}

StringDictionaryMMPFCPLAINLlcp::StringDictionaryMMPFCPLAINLlcp(IteratorDictString *it)
{
...
uint 
StringDictionaryMMPFCPLAINLlcp::locate(uchar *str, uint strLen)
{
...
uchar *
StringDictionaryMMPFCPLAINLlcp::extract(size_t id, uint *strLen)
{
...
IteratorDictID*
StringDictionaryMMPFCPLAINLlcp::locatePrefix(uchar *str, uint strLen)
{
...
IteratorDictID*
StringDictionaryMMPFCPLAINLlcp::locateSubstr(uchar *str, uint strLen)
{
...
uint 
StringDictionaryMMPFCPLAINLlcp::locateRank(uint rank)
{
	return rank;
}

IteratorDictString*
StringDictionaryMMPFCPLAINLlcp::extractPrefix(uchar *str, uint strLen)
{
...
IteratorDictString*
StringDictionaryMMPFCPLAINLlcp::extractTable()
{
...
IteratorDictString*
StringDictionaryMMPFCPLAINLlcp::extractSubstr(uchar *str, uint strLen)
{
...
uchar *
StringDictionaryMMPFCPLAINLlcp::extractRank(uint rank, uint *strLen)
{
	return extract(rank, strLen);
}

size_t 
StringDictionaryMMPFCPLAINLlcp::getSize()
{
	return (bytesStrings*sizeof(uchar))+(this->realElements+1)*sizeof(uint)+llcp->getSize()+sizeof(StringDictionaryMMPFCPLAINLlcp);
}

void 
StringDictionaryMMPFCPLAINLlcp::save(ofstream &out)
{
...
StringDictionary*
StringDictionaryMMPFCPLAINLlcp::load(ifstream &in)
{
	size_t type = loadValue<uint32_t>(in);
	if(type != MMPFCPLAINLLCP) return NULL;

	cout << "Reading MMPFCPLAINLlcp" <<endl;

	StringDictionaryMMPFCPLAINLlcp *dict = new StringDictionaryMMPFCPLAINLlcp();

	dict->type = MMPFCPLAINLLCP;
...
//void
//StringDictionaryMMPFCPLAINLlcp::decodeNextString(uchar **ptr, uint lenPrefix, uchar *str, uint *strLen)
//{
//	uint lenSuffix;
//
...
//uint
//StringDictionaryMMPFCPLAINLlcp::searchPrefix(uchar **ptr, uint scanneable, uchar *decoded, uint *decLen, uchar *str, uint strLen)
//{
...
//
//uint
//StringDictionaryMMPFCPLAINLlcp::searchDistinctPrefix(uchar *ptr, uint scanneable, uchar *decoded, uint *decLen, uchar *str, uint strLen)
//{
//	uint id = 1;
//	uint lenPrefix;
//
...
void StringDictionaryMMPFCPLAINLlcp::printInfo() {
...
//    printf("XBWInt -> %lu\n", this->xbw->getSize());
//    printf("----------------------------\n");
    printf("MMPFC structure: %lu\n", sizeof(StringDictionaryMMPFCPLAINLlcp));
    printf("TOTAL  -> %lu\n", this->getSize());
//    printf("****************************\n");
//    printf("\n\n");
//
...
void StringDictionaryMMPFCPLAINLlcp::buildLcps(uchar *dict, uint *pointers, vector<size_t>*llcp, vector<size_t>*rlcp, uint l, uint r) {
    uint m = (l + r) / 2;
//    uint level = bits(r-l);
//    if (level > 10)
...
inline int StringDictionaryMMPFCPLAINLlcp::doSearchString(uchar * string, uint len, uint lpos, uint rpos, uint l, uint r) {
    uint m = (lpos + rpos) / 2;
    uint foo;
...
inline int StringDictionaryMMPFCPLAINLlcp::compareNewPattern(uchar *pattern, uchar *compared, uint *pos, int len) {
	uint p=*pos;
